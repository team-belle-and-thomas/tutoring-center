...
export function ParentProgressDashboard({
  ...
  const filteredData = useMemo(() => {
    ...
    if (!isAllSubjects) {
      performance = performance.filter(p => p.subject === selectedSubject);
      confidence = confidence.filter(c => c.subject === selectedSubject);
      homework = homework.filter(h => h.subject === selectedSubject);
    }

    if (dateRange !== 'all') {
      performance = performance.filter(p => isWithinDateRange(p.date, dateRange));
      confidence = confidence.filter(c => isWithinDateRange(c.date, dateRange));
      homework = homework.filter(h => isWithinDateRange(h.date, dateRange));
    }

    const filteredGrades = isAllSubjects
      ? initialGrades
      : initialGrades.filter(g => g.subject.toLowerCase() === selectedSubject.toLowerCase());
