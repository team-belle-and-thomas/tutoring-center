...
export async function getAdminMetrics(): Promise<AdminMetrics> {
  ...
  const pendingNotesCreditsAtRisk = pendingNotes.reduce((sum, s) => sum + s.slot_units, 0);
  ...
  type CompletedRow = {
    id: number;
    slot_units: number;
    credit_transactions: Array<{ id: number; amount: number }> | null;
  };
  const completedSessions = (completedSessionsResult.data ?? []) as CompletedRow[];

  const creditsLeaked = completedSessions
    .filter(s => !(s.credit_transactions ?? []).some(tx => tx.amount < 0))
    .reduce((sum, s) => sum + s.slot_units, 0);
...
export async function getAtRiskParents(): Promise<AtRiskParent[]> {
  ...
  return data.flatMap(row => {
    const parentRaw = Array.isArray(row.parents) ? row.parents[0] : row.parents;
    if (!parentRaw) return [];
    const userRaw = parentRaw.users ? pickFirstEmbedded(parentRaw.users) : null;
    if (!userRaw) return [];
    const user = userRaw as { first_name: string | null; last_name: string | null; email: string };
    return [
      {
        parent_id: parentRaw.id,
